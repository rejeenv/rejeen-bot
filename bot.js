...
// DISCORD_BOT_TOKEN, DISCORD_CLIENT_ID, UPSTASH_REDIS_REST_URL, UPSTASH_REDIS_REST_TOKEN
...
const EXPIRE_SECONDS = 60 * 60 * 24 * 3; // 3 dni
...
async function registerCommand() {
  try {
    ...
    console.log('✅ Komenda /rejeen zarejestrowana');
  } catch (err) {
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.commandName !== 'rejeen') return;
  ...
  // Walidacja pól dla każdego typu
  const errors = [];
  if (typ === 'rockstar') {
  ...
  try {
    ...
    // Użytkownik może mieć zablokowane DM
